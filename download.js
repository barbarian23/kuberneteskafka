...
app.get('/testMysql', async function (req, res) {

    let interQuerry = setInterval(() => {
        try{
        const mysql = require('mysql2');
        const connection = mysql.createConnection({
            host: "20.239.4.165",
            user: "root",
            password: "ZGViZXppdW1zb3VyY2U=",
            database: "debeziumdb"
        });

        connection.connect();

        connection.query("insert into debeziumdb.employee(name,age) values  (\"harry123456\",30);", function (error, results, fields) {
            if (error) {
...
const runMysql = (url, user, pasword, database, query) => {
